# Secure Crawler— Architecture Decisions
...
**Limitation:** *Family-based matching still detects string presence, not semantic role. It cannot distinguish the model reporting a blocked/attempted injection from executing one. Closing this would require semantic/intent analysis, out of scope for this project. Ratio-based justification: benign summaries are unlikely to incidentally contain compromise-indicator phrasing, so the false-negative-on-reported-attempts case is accepted, not solved.*
